...
// Offline dev
const _BASEURI = "https://localhost:7071/api";
    ...
    for (let htmlCard of htmlCards) {
        ...
        htmlCard.addEventListener("click", function() {
            if (!alreadyPicked) {
                ...
                voteCard(htmlCard.dataset.id, htmlCard.innerHTML, htmlCard.dataset.hot);
            }
            else {
                console.log("Already picked!")
            }
...
const voteCard = function(id, text, hot) {
    let card = {}
    card.id = id;
    card.text = text;
    card.hot = hot;
